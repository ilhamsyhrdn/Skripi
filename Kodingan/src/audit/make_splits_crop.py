"""Same as make_splits.py but for the cropped-image pool (lidc_canonical_pool_crop.csv)."""
...
IN_CSV = Path("D:/skripsi/Kodingan/outputs/manifests/lidc_canonical_pool_crop_relabeled.csv")
TEST_CSV = Path("D:/skripsi/Kodingan/outputs/manifests/test_holdout_crop.csv")
TRAINVAL_CSV = Path("D:/skripsi/Kodingan/outputs/manifests/trainval_folds_crop.csv")
N_FOLDS = 5
TEST_FRAC = 0.18
SEED = 42
...
def main():
    ...
    print(f"Canonical pool (crop): {len(df)} images, {df['split_group'].nunique()} patients", flush=True)
